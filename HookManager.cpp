...
DWORD WINAPI HookThread(LPVOID param) {
	...
	while (true) {
		for (int i = 0; i < hmanager->hooks.size(); i++) {
			if (find(i, &completed))
				continue;
			if (!SignatureScanner::FindSignature(&hookLocation, 0x30000000, 0x01000000, hmanager->hooks.at(i).hookString, hookMask, 0)) {
				continue;
			}
			...
			*(DWORD*)(hookLocation + 1) = ((DWORD)hmanager->hooks.at(i).functionPointer - hookLocation - 5);
...
void HookMember::Hook() {
	...
	hookString = new char[14] {
		(char)0x8B, (char)0x0D, (char)0xC0, (char)0x9B, (char)0x8D, (char)0x0, (char)0x81, (char)0xC1, (char)handle, (char)0x13, (char)0x0, (char)0x0, (char)0x89, (char)0xC8
	};
	bytesToReplace = new char[4] {
		(char)handle, (char)0x13, (char)0x9B, (char)0x8D
	};
